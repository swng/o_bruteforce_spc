...
async function generate(base_field, hold_piece) {
    ...
    for (let col = 0; col < 10; col++) {
        for (let row = 0; row < 6; row++) {
            for (let rotation_state of ['spawn']) { // only one rotation state needed for O
...
async function main() {
    ...
    let results = await generate(empty_field, 'O');
    
    let index = 0;
    for (let result of results) { // 9 initial O placements
        console.log(result);
        // java -jar sfinder.jar spin -p '*p7' -fb 0 -ft 4 -m 6 -c 1 -t 'v115@Rhg0Iei0QeAgH'
        let command = `java -jar sfinder.jar spin -p '*p7' -fb 0 -ft 4 -m 6 -c 1 -t ${result} -o spin_files/o_spc_${String(index).padStart(2, '0')}.html`
        // console.log(command);
